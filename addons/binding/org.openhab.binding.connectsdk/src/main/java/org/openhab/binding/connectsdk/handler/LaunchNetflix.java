 ...
 * @since 2.1.0
 */
public class LaunchNetflix extends AbstractChannelHandler<Launcher.AppInfoListener> {
    ...
    public void onReceiveCommand(final ConnectableDevice d, Command command) {
        if (d.hasCapabilities(Launcher.Netflix)) {
            final String value = command.toString();
            final Launcher control = getControl(d);
            control.launchNetflix(value, LaunchNetflix.this.<LaunchSession>createDefaultResponseListener());
        }
    }
