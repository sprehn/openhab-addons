 ...
 * @since 2.1.0
 */
public class LaunchYoutube extends AbstractChannelHandler<Launcher.AppInfoListener> {
    ...
    public void onReceiveCommand(final ConnectableDevice d, Command command) {
        if (d.hasCapabilities(Launcher.YouTube)) {
            final String value = command.toString();
            final Launcher control = getControl(d);
            control.launchYouTube(value, LaunchYoutube.this.<LaunchSession>createDefaultResponseListener());
        }
    }
