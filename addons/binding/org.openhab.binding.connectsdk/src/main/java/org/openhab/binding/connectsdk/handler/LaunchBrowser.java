 ...
 * @since 2.1.0
 */
public class LaunchBrowser extends AbstractChannelHandler<Launcher.AppInfoListener> {
    ...
    public void onReceiveCommand(final ConnectableDevice d, Command command) {
        if (d.hasCapabilities(Launcher.Browser)) {
            final String value = command.toString();
            final Launcher control = getControl(d);
            control.launchBrowser(value, LaunchBrowser.this.<LaunchSession>createDefaultResponseListener());
        }
    }
