...
public class ServiceDescription implements Cloneable {
    ...
    public ServiceDescription(JsonObject json) {
        serviceFilter = Util.optString(json, KEY_FILTER);
        ipAddress = Util.optString(json, KEY_IP_ADDRESS);
        UUID = Util.optString(json, KEY_UUID);
        friendlyName = Util.optString(json, KEY_FRIENDLY);
        modelName = Util.optString(json, KEY_MODEL_NAME);
        modelNumber = Util.optString(json, KEY_MODEL_NUMBER);
        port = json.has(KEY_PORT) ? json.get(KEY_PORT).getAsInt() : -1;
        version = Util.optString(json, KEY_VERSION);
        serviceID = Util.optString(json, KEY_SERVICE_ID);
    }
    ...
    private void putOpt(JsonObject json, String key, String value) {
        if (value != null) {
    ...
    public JsonObject toJSONObject() {
        ...
        putOpt(jsonObj, KEY_FILTER, serviceFilter);
        putOpt(jsonObj, KEY_IP_ADDRESS, ipAddress);
        putOpt(jsonObj, KEY_UUID, UUID);
        putOpt(jsonObj, KEY_FRIENDLY, friendlyName);
        putOpt(jsonObj, KEY_MODEL_NAME, modelName);
        putOpt(jsonObj, KEY_MODEL_NUMBER, modelNumber);
        jsonObj.addProperty(KEY_PORT, port);
        putOpt(jsonObj, KEY_VERSION, version);
        putOpt(jsonObj, KEY_SERVICE_ID, serviceID);
