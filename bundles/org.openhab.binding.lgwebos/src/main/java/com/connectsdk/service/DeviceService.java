...
/**
 * ###Overview
 * From a high-level perspective, DeviceService completely abstracts the functionality of a particular service/protocol
 * (webOS TV, Netcast TV, Chromecast, Roku, DIAL, etc).
 *
 * ###In Depth
 * DeviceService is an abstract class that is meant to be extended. You shouldn't ever use DeviceService directly,
 * unless extending it to provide support for an additional service/protocol.
 *
 * Immediately after discovery of a DeviceService, DiscoveryManager will set the DeviceService's Listener to the
 * ConnectableDevice that owns the DeviceService. You should not change the Listener unless you intend to manage the
 * lifecycle of that service. The DeviceService will proxy all of its Listener method calls through the
 * ConnectableDevice's ConnectableDeviceListener.
 *
 * ####Connection &amp; Pairing
 * Your ConnectableDevice object will let you know if you need to connect or pair to any services.
 *
 * ####Capabilities
 * All DeviceService objects have a group of capabilities. These capabilities can be implemented by any object, and that
 * object will be returned when you call the DeviceService's capability methods (launcher, mediaPlayer, volumeControl,
 * etc).
 */
public class DeviceService implements ServiceCommandProcessor {
    ...
    // public SparseArray<ServiceCommand<? extends Object>> requests = new SparseArray<ServiceCommand<? extends
    // Object>>();
    ...
    public static DeviceService getService(JsonObject json) {
        ...
        try {
            String className = json.get(KEY_CLASS).getAsString();
            ...
            JsonObject jsonConfig = json.get(KEY_CONFIG).getAsJsonObject();
            ServiceConfig serviceConfig = null;
            if (jsonConfig != null) {
                serviceConfig = ServiceConfig.getConfig(jsonConfig);
            }

            JsonObject jsonDescription = json.get(KEY_DESC).getAsJsonObject();
    ...
    public static DeviceService getService(Class<? extends DeviceService> clazz, ServiceDescription serviceDescription,
            ServiceConfig serviceConfig) {
        try {
            Constructor<? extends DeviceService> constructor = clazz.getConstructor(ServiceDescription.class,
                    ServiceConfig.class);
    ...
    /**
     * Will attempt to connect to the DeviceService. The failure/success will be reported back to the
     * DeviceServiceListener. If the connection attempt reveals that pairing is required, the DeviceServiceListener will
     * also be notified in that event.
     */
    public void connect() {
    ...
    /**
     * Will attempt to disconnect from the DeviceService. The failure/success will be reported back to the
    ...
    public JsonObject toJSONObject() {
        ...
        jsonObj.addProperty(KEY_CLASS, getClass().getSimpleName());
    ...
    /**
     * Closes the session on the first screen device. Depending on the sessionType, the associated service will have
    ...
    public void closeLaunchSession(LaunchSession launchSession, ResponseListener<Object> listener) {
        if (launchSession == null) {
            Util.postError(listener, new ServiceCommandError("You must provide a valid LaunchSession"));
            return;
        }

        DeviceService service = launchSession.getService();
        if (service == null) {
            Util.postError(listener, new ServiceCommandError("There is no service attached to this launch session"));
            return;
        }
        ...
        switch (launchSession.getSessionType()) {
            ...
            default:
                Util.postError(listener,
                        new ServiceCommandError("This DeviceService does not know ho to close this LaunchSession"));
                break;
        }
    }
    ...
    public interface DeviceServiceListener {
        ...
        /*
         * !
         * There are situations in which a DeviceService will update the capabilities it supports and propagate these
         * changes to the DeviceService. Such situations include:
         ...
         *
         * For more information on this particular method, see ConnectableDeviceDelegate's
        ...
        /*
         * !
         * This method will be called on any disconnection. If error is nil, then the connection was clean and likely
         * triggered by the responsible DiscoveryProvider or by the user.
         *
         * @param service DeviceService that disconnected
         *
         * @param error Error with a description of any errors causing the disconnect. If this value is nil, then the
         * disconnect was clean/expected.
         */
        ...
        /*
         * !
         * This method will be called upon pairing success. On pairing success, a connection to the DeviceService will
         * be attempted.
         *
